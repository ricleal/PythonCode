...
def test_linear_least_squares():
    ...
    print "Custom y=m*x+c: m=%.2f , c=%.2f"%(m,c)
    
    m_d, c_d = np.linalg.lstsq(A, y)[0]
    print "Default y=m*x+c: m=%.2f , c=%.2f"%(m_d,c_d)
    
    import matplotlib.pyplot as plt

    plt.plot(x, y, '.', label='Original data', markersize=10)
    plt.plot(x, m * x + c, 'r', label="Custom y=%.2f*x+%.2f"%(m,c))
    plt.plot(x, m_d * x + c_d, 'g', label="Default y=%.2f*x+%.2f"%(m_d,c_d))
