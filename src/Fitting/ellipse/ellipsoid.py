...
NOTE: it doesn't fit an ellipsoid. 

It calculates an ellipsoide that contains all the points:  Bounding ellipse
...
if __name__ == "__main__":
    ...
    timeit()
    mean = [0,0,0]
    cov = [[40,0,0],[0,60,0],[0,0,90]] # diagonal covariance, points lie on x or y-axis
    P = np.random.multivariate_normal(mean,cov,5000)
    timeit()
    # find the ellipsoid
    ET = EllipsoidTool()
    (center, radii, rotation) = ET.getMinVolEllipse(P, 0.01)
    timeit()
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')

    # plot points
    ax.scatter(P[:,0], P[:,1], P[:,2], color='g', marker='.')
