...
"""
Thread manager with timeout-based lifecycle control.

Launches threaded function calls (via eval) and monitors their execution.
Threads that exceed a configurable timeout are automatically flagged for
removal. Demonstrates thread lifecycle management, custom Thread subclass,
and periodic cleanup in a monitoring loop.
"""
...
class ThreadManager(threading.Thread):
    ...
    def _isThreadEntryToDelete(self, entry):
        if (
            entry["thread"].is_alive()
            and (time.time() - entry["startTime"] > self._timeOut)
    ...
    def run(self):
        while True and not self._exitFlag:
            ...
            self._threadingList[:] = [
                x for x in self._threadingList if not self._isThreadEntryToDelete(x)
            ]
            time.sleep(0.2)
    ...
    def addThread(self, functionSignatureToCall):
        ...
        startTime = time.time()
        entry = {"startTime": startTime, "thread": t}
        self._threadingList.append(entry)
        t.start()

    def exit(self):
        print("Exiting: threads running:" + str(self))
        self._exitFlag = True
